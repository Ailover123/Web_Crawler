...
class URLPolicy:
    ...
    _lock: Lock = Lock()
    _allowed_domain: str = None  # Strict domain scope
    _stats: Dict[str, int] = {
        "evaluations": 0,
        "allowed": 0,
        "blocked_non_http": 0,
        "blocked_external": 0,  # New stat for domain checks
        "blocked_fragment": 0,
        "blocked_asset": 0,
    ...
    @classmethod
    def set_allowed_domain(cls, domain: str) -> None:
        """
        Configure the strict domain scope for the session.
        Only URLs matching this netloc will be allowed.
        """
        with cls._lock:
            cls._allowed_domain = domain

    @classmethod
    def eval(cls, url: str):
        """
        Evaluate a URL and return (allowed: bool, reason: str).
        Reasons are one of the stats keys: 'allowed', 'blocked_non_http', 'blocked_external',
        'blocked_fragment', 'blocked_asset', 'blocked_path_system', 'blocked_path_taxonomy', 
        'blocked_substring'.
        ...
        try:
            ...
            with cls._lock:
                # If set_allowed_domain was called, strictly enforce it
                if cls._allowed_domain and netloc != cls._allowed_domain:
                    cls._stats["blocked_external"] += 1
                    return False, "blocked_external"
        except Exception:
            return False, "blocked_external"
